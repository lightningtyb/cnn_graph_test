...
class TextDataset(object):
    ...
    def data_info(self, show_classes=False):
        ...
        out3="out3--paran="+str(show_classes)+"---N="+str(N)+"documents,M="+str(M)+"words,sparsity="+str(sparsity)+'\n\n'
        with open('output1.txt', 'a+') as writter:
            writter.write(str(out3))

        if show_classes:
            for i in range(len(self.class_names)):
                num = sum(self.labels == i)
                print('  {:5d} documents in class {:2d} ({})'.format(num, i, self.class_names[i]))
                out4='out4---i='+str(i)+'--'+str(num)+'documents in class'+str(self.class_names[i])+'\n\n'
                with open('output1.txt', 'a+') as writter:
                    writter.write(str(out4))
    ...
    def show_document(self, i):
        ...
        out='out--def show_doc in utils--document '+str(i)+': label '+str(label)+'--> '+str(name)+', '+str(wc)+' words'+'\n\n'
        with open('output1.txt', 'a+') as writter:
            writter.write(str(out))
        ...
        try:
            ...
            for j in range(vector.shape[1]):
                if vector[0,j] != 0:
                    ...
                    out1 = 'out--def show_doc in utils-- '+str(vector[0,j])+ '" '+str(self.vocab[j])+ '" ('+str(j)+')'
                    with open('output1.txt', 'a+') as writter:
    ...
    def keep_top_words(self, M, Mprint=20):
        ...
        out6='out6--def kpTopWords in utils ---most frequent words'+'\n\n'
        with open('output1.txt', 'a+') as writter:
            writter.write(str(out6))
        for i in range(Mprint):
            print('  {:3d}: {:10s} {:6d} counts'.format(i, self.vocab[i], freq[idx][i]))
            out7='out7--i='+str(i)+': '+str(self.vocab[i])+str(freq[idx][i])+' counts'+'\n\n'
            with open('output1.txt', 'a+') as writter:
                writter.write(str(out7))
...
class Text20News(TextDataset):
    def __init__(self, **params):
        ...
        N, C = len(self.documents), len(self.class_names)# N是文章数目，C是种类数目
        print('N = {} documents, C = {} classes'.format(N, C))
        # print('class Text20News ---\ndataset.data={}'.format(self.documents))
        print('class Text20News ---\ndataset.target={}'.format(self.labels))#dataset.target=[7 4 4 ..., 3 1 8]每次不一样
        out0='out0---class Text20News --def init---\ndataset.target={}'+str(self.labels)
        out0+='\n\n'
        with open('output1.txt', 'a+') as writter:
            writter.write(str(out0))
        print('class Text20News ---\ndataset.target_name={}'.format(self.class_names))#就是那个20个种类
        out01 = 'out01 in class text20News'+str(self.class_names)+'\n\n'
        with open('output1.txt', 'a+') as writter:
            writter.write(str(out01))
